...
GRANT USAGE ON SCHEMA market_data, market_insights TO ep_senex, ep_senex_r;
GRANT USAGE ON SCHEMA public TO ep_senex;

GRANT SELECT ON ALL TABLES IN SCHEMA market_data, market_insights TO ep_senex_r;
ALTER DEFAULT PRIVILEGES IN SCHEMA market_data, market_insights GRANT SELECT ON TABLES TO ep_senex_r;

GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON ALL TABLES IN SCHEMA market_data, market_insights TO ep_senex;
ALTER DEFAULT PRIVILEGES IN SCHEMA market_data, market_insights GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON TABLES TO ep_senex;
...
CREATE TABLE market_insights.news_articles (
	...
	created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
	updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
	CONSTRAINT news_pk PRIMARY KEY (url)
);
